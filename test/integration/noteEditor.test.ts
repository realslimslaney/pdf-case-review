...
// the debug seam mutate the model exactly like the real webview would.
...
suite("M2 phase 2: note editor view", () => {
  ...
  test("editNote targets a highlight and saveNote persists through the view", async () => {
    ...
    await postToEditor({ type: "saveNote", target, note: "Key tension." });
  ...
  test("setCategory through the view updates the model", async () => {
    ...
    const id = state?.model.highlights[0]?.id;
    assert.ok(id);
    await postToEditor({ type: "setCategory", target: { kind: "highlight", id }, categoryId: "question" });
  ...
  test("messages for a stale target are dropped", async () => {
    const state = await documentState(pdf);
    const id = state?.model.highlights[0]?.id;
    assert.ok(id);
    await postToEditor({ type: "saveNote", target: { kind: "highlight", id: "not-the-target" }, note: "x" });
  ...
  test("deleteTarget removes a document note and clears the view", async () => {
    ...
    await postToEditor({ type: "deleteTarget", target: { kind: "document", id } });
