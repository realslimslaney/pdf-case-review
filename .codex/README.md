...
| Docs upkeep | `.claude/agents/docs-maintainer.md` | `.agents/skills/docs-maintainer/` and `.codex/agents/docs-maintainer.toml` |
| PDF.js upgrades | `.claude/agents/pdfjs-upgrader.md` | `.agents/skills/pdfjs-upgrader/` and `.codex/agents/pdfjs-upgrader.toml` |
| Post a plan to GitHub | `.claude/skills/post-plan/` | `.agents/skills/post-plan/` |
| Style guard (em-dash, US spelling) | `.claude/settings.json` → `.claude/hooks/style_guard.py` | `.codex/hooks.json` → the same script |

Why `--allow-direct`: Claude hook payloads identify the calling subagent, so the gate can insist that only the `committer` agent commits. Codex payloads have no such field, so for Codex the gate drops that one rule and keeps the rest (never on `main`, never `--no-verify`, no em-dash in the message, `package.json` staged alongside a staged `pnpm-lock.yaml`, green `pnpm run test:unit`). The Codex committer skill carries the "explicit approval quoted verbatim" rule in prose instead.
...
The hooks need a `python` on PATH (any 3.10+; stdlib only). `scripts/check-docs.mjs` (`pnpm docs:check`, also in CI) enforces that these mirrors stay present and that shared skills stay byte-identical between `.claude/skills/` and `.agents/skills/`. No secrets or machine-specific settings belong in this folder.
