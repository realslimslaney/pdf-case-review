...
|---|---|
| `PDF Case Review: Choose AI Provider...` | Probes the `claude` and `codex` CLIs and sets `pdfCaseReview.ai.provider`; unavailable options show a one-line fix. Also reached by clicking the AI status bar item, and from the **Switch AI Provider...** button on a failed run's error notification (picking a different CLI there reruns the command; a failure that reads like a usage limit is reported as such, with the raw error in the output channel). Each CLI is probed under the login the active document's rules select (else the default login), so the email shown is the one the consent dialog will verify: `✓ you@school.edu · HBS · account "school"`, `✓ installed, not signed in under ~/.codex-school (account "school")`, or `✓ installed · no Codex login registered as "school"` (allowed; the confirmation then offers **Add an AI Account...**). With no PDF open the default logins are shown. Picking Manual starts the clipboard flow. Switching providers re-asks the consent question on the next run. |
...
Next to it, in a trusted workspace, the AI item shows the provider and the account the active document would run under: `$(sparkle) AI off`, `$(sparkle) Claude Code` (default login), `$(sparkle) Claude Code · school` (the `ai.accounts` entry a rule selected), `$(sparkle) Codex · rule pending` (a rule needs the document's authorization line and the viewer has not loaded it yet), or `$(sparkle) Claude Code · desktop only` on the web host. A warning background marks a run that would be refused as configured: `$(warning) Codex · no "school" login` (the id has no entry for the active provider) or `$(warning) Codex · "x" unknown` (the id is in no `ai.accounts` entry). The tooltip adds the login directory, the signed-in email and organization probed from it, and the rule that selected the account. Clicking it runs **Choose AI Provider...**.
