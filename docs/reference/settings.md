...
Nothing leaves your machine unless a provider is enabled or you copy the prompt yourself. By default only highlights and notes are sent; the `ai.contextScope` setting can additionally send text extracted from the document, and the PDF file itself is never sent. Every run passes the eligibility question naming the signed-in account. See [the AI reviewer how-to](../how-to/ai-reviewer.md).
...
| `pdfCaseReview.ai.contextScope` | `notes` | What AI features may send. `notes`: highlights and notes only; the PDF's own text is never sent. `document-text`: **Summarize with AI** and **Copy Summary Prompt** also send the document text, extracted per page with citations under a 400,000-character budget (truncation is reported explicitly). Image-only pages and scans have no extractable text and are excluded; the PDF file itself is never sent under either scope. Changing the scope always re-asks the consent question, and the dialog states exactly what will be sent with coverage numbers. **Add AI Page Context** always sends notes only. |
