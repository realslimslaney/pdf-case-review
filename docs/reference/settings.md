...
| `pdfCaseReview.ai.requiredAccount` | `[]` | Rules refusing the AI step under the wrong login. The first rule whose `when` matches (`protected`, `authorizationLineMatches`, `pathGlob`) applies; `email` names the account that must be signed in (no override) and `use` selects the `ai.accounts` entry with that id for the active provider. Leave `email` off a rule with `use` when the logins registered under that id have different addresses. |
| `pdfCaseReview.ai.accounts` | `[]` | Separate CLI login directories (`{id, provider, configDir}`) for people with more than one account; the extension sets `CLAUDE_CONFIG_DIR` / `CODEX_HOME` on the spawned CLI itself. An `id` is unique per provider, not globally: the same id may appear once for `claude-cli` and once for `codex-cli`, and a rule's `use` picks the entry for the active `pdfCaseReview.ai.provider`, so switching the provider switches the login directory with it. A matched rule whose id has no entry for the active provider is refused with an error naming the fix (its **Add an AI Account...** button opens the guided flow with provider and id preset); an id unknown to every provider is refused as a configuration error. A repeated (id, provider) pair warns and the first entry wins. The gate and the spawn share one lookup, so a run can never execute under a different CLI or login than the one the consent dialog verified. |
