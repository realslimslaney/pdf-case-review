...
- **Skills** are in `.agents/skills/` (Codex discovers them from the working directory up to the repo root): `committer` (stage and commit approved work), `pr-manager` (open or repair a draft PR), `post-plan` (turn a plan into a GitHub issue or PR comment). Custom-agent equivalents of the first two live in `.codex/agents/` for sessions that use Codex sub-agents.
- **Committing.** Propose the commit, wait for the user's explicit yes, then follow `.agents/skills/committer`. A PreToolUse hook in `.codex/hooks.json` runs the shared commit gate (`.claude/hooks/gate_commit.py --allow-direct`) on every `git commit`; it blocks commits on `main`, `--no-verify`, a staged `pnpm-lock.yaml` without a `package.json` version bump, and a red `pnpm run test:unit`. Fix the cause; never work around it. Claude Code runs the same gate through `.claude/settings.json`.
- **Pull requests.** When a pushed branch is ready, use `.agents/skills/pr-manager`: draft PR, assignee `realslimslaney`, one `area:*` label, `Closes #N`, Codex attribution footer. Never create labels.
